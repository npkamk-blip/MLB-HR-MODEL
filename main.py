...
function switchTab(id,btn){
  ...
  if(id==='dashboard'&&!dashboardData) loadDashboard();
...
function buildDashboard(d){
  ...
  var trackStart=stats.tracking_start||'2026-05-07';
  h+=sCard('Days Tracked',daysT,'Since '+trackStart,'var(--text)');
  h+='</div>';
...
function buildBatterTable(games){
  ...
  games.forEach(function(g){
    var wx=g.weather||{};
    var wxLbl=(wx.label||'').toLowerCase();
    var windS=wx.wind_speed||0;
    var wxMult=1.0;
    if(!wxLbl.includes('dome')&&windS>=5){
      var al=labelToAlign(wx.label||'');
      var sf=windS<10?0.3:windS<16?0.7:1.0;
      wxMult=1.0+(al*sf*0.12);
    }
    var pf=g.park_factor||g.away_pitcher&&g.away_pitcher.park_factor||1.0;
    var awayP=g.away_pitcher||{};
    var homeP=g.home_pitcher||{};
    var awayP2=g.away_pitcher||{};var homeP2=g.home_pitcher||{};
    var aHr9=awayP2.hr9||1.2; var hHr9=homeP2.hr9||1.2;
    var aIP=awayP2.avg_ip||5.5; var hIP=homeP2.avg_ip||5.5;
    var aBpHr9=awayP2.bullpen_hr9||1.2; var hBpHr9=homeP2.bullpen_hr9||1.2;
    var projHR=Math.round(((hHr9/9)*hIP+(hBpHr9/9)*(9-hIP)+(aHr9/9)*aIP+(aBpHr9/9)*(9-aIP))*pf*wxMult*10)/10;
    totalProjHR+=projHR;
  });
  var slateColor=totalProjHR>=32?'var(--g)':totalProjHR>=22?'var(--y)':'var(--muted)';
  var slateLabel=totalProjHR>=32?'strong day':totalProjHR>=22?'average day':'low HR day';
  h+='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;padding:0 2px">';
  h+='<div style="font-size:11px;color:var(--muted2)">'+games.length+' games today</div>';
  h+='<div style="font-size:11px;color:var(--muted2)">Slate total: <span style="color:'+slateColor+';font-weight:600">'+totalProjHR.toFixed(0)+' proj. HRs</span> - '+slateLabel+'</div>';
  h+='</div>';

  // ── Column header ──
  h+='<div style="display:grid;grid-template-columns:210px 75px 85px 1fr 1fr;gap:0;border-radius:6px 6px 0 0;overflow:hidden;border:0.5px solid var(--border)">';
  ['Game','Proj HRs','Wind','Away Batters','Home Batters'].forEach(function(lbl){
    h+='<div style="background:var(--s2);padding:5px 8px;font-size:9px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:.07em;border-bottom:0.5px solid var(--border)">'+lbl+'</div>';
  });
  h+='</div>';
  ...
  games.forEach(function(g,gi){
    ...
    var pf=awayP.park_factor||homeP.park_factor||g.park_factor||1.0;
    var wxLabel=(wx.label||'').toLowerCase();
    var windSpd=wx.wind_speed||0;
    var isDome=wxLabel.includes('dome');
    // Use backend-calculated mult when available, otherwise approximate from label
    var wxMult=wx.mult||1.0;
    if(!wx.mult && !isDome && windSpd>=5){
      var align=labelToAlign(wx.label||'');
      var speedFactor=windSpd<10?0.3:windSpd<16?0.7:1.0;
      wxMult=1.0+(align*speedFactor*0.12);
    }

    var allCands=g.top_hr_candidates||[];
    var awayBatters=allCands.filter(function(b){return b.team===awayTeam;}).sort(function(a,b){return(b.hr_prob||0)-(a.hr_prob||0);});
    var homeBatters=allCands.filter(function(b){return b.team===homeTeam;}).sort(function(a,b){return(b.hr_prob||0)-(a.hr_prob||0);});

    if(!awayBatters.length&&!homeBatters.length)return;
    totalBatters+=awayBatters.length+homeBatters.length;

    // ── Projected HR: starter innings + bullpen innings ──
    var awayHr9=awayP.hr9||1.2;
    var homeHr9=homeP.hr9||1.2;
    var awayIP=awayP.avg_ip||5.5;
    var homeIP=homeP.avg_ip||5.5;
    var awayBpHr9=awayP.bullpen_hr9||1.2;
    var homeBpHr9=homeP.bullpen_hr9||1.2;
    var awayBpIP=Math.max(0, 9-awayIP);
    var homeBpIP=Math.max(0, 9-homeIP);
    // away batters face home pitcher + home bullpen
    var awayStarterProj=(homeHr9/9)*homeIP*pf*wxMult;
    var awayBullpenProj=(homeBpHr9/9)*homeBpIP*pf*wxMult;
    var awayProj=Math.round((awayStarterProj+awayBullpenProj)*10)/10;
    // home batters face away pitcher + away bullpen
    var homeStarterProj=(awayHr9/9)*awayIP*pf*wxMult;
    var homeBullpenProj=(awayBpHr9/9)*awayBpIP*pf*wxMult;
    var homeProj=Math.round((homeStarterProj+homeBullpenProj)*10)/10;
    var gameProj=Math.round((awayProj+homeProj)*10)/10;
    var projC=gameProj>=2.8?'var(--g)':gameProj>=2.0?'var(--y)':'var(--muted)';
    var projLabel=gameProj>=2.8?'IDEAL':gameProj>=2.0?'NORMAL':'LOW';

    // ── Wind direction arrow (rotated SVG) ──
    function windArrow(wx,isDome){
      if(isDome)return'<div style="text-align:center;font-size:10px;color:var(--muted2)">Dome</div>';
      var wind=wx.wind_speed||0;
      var label=wx.label||'';
      var l=label.toLowerCase();
      if(wind<4)return'<div style="text-align:center;font-size:10px;color:var(--muted2)">Calm</div>';
      var align=labelToAlign(label);
      var arrowC=align>=0.5?'var(--g)':align>=0.1?'var(--y)':align<=-0.4?'var(--r)':'var(--muted2)';
      // Determine rotation - 0=up=out to CF, 180=down=blowing in
      var rot=0;
      if(l.includes('blowing in')||l.includes('slightly in'))rot=180;
      else if(l.includes('out to lf'))rot=-45;
      else if(l.includes('out to rf'))rot=45;
      else if(l.includes('toward lf'))rot=135;
      else if(l.includes('toward rf'))rot=-135;
      else if(l.includes('cross'))rot=90;
      var shortLabel=l.includes('out')?'OUT':l.includes('in')?'IN':'CROSS';
      return'<div style="display:flex;flex-direction:column;align-items:center;gap:3px">'
        +'<svg width="32" height="32" viewBox="0 0 32 32" style="transform:rotate('+rot+'deg)">'
        +'<rect x="6" y="6" width="20" height="20" rx="2" fill="none" stroke="rgba(255,255,255,0.15)" stroke-width="1" transform="rotate(45 16 16)"/>'
        +'<circle cx="16" cy="16" r="2" fill="rgba(255,255,255,0.3)"/>'
        +'<line x1="16" y1="16" x2="16" y2="4" stroke="'+arrowC+'" stroke-width="2" stroke-linecap="round"/>'
        +'<polygon points="16,1 13,7 19,7" fill="'+arrowC+'"/>'
        +'</svg>'
        +'<div style="font-size:10px;font-weight:600;color:'+arrowC+'">'+wind+'mph '+shortLabel+'</div>'
        +'</div>';
    }

    // ── Pitcher info with HR/9 splits ──
    function pitcherInfo(p){
      if(!p||!p.name)return'<div style="font-size:11px;color:var(--muted2)">TBD</div>';
      var hand=p.hand||'R';
      var handCls=hand==='L'?'lhp':'rhp';
      var rhr9=p.vs_R_hr9||0;
      var lhr9=p.vs_L_hr9||0;
      var rCls=hr9Cls(rhr9);
      var lCls=hr9Cls(lhr9);
      return'<div style="margin-bottom:5px">'
        +'<div style="display:flex;align-items:center;gap:4px;margin-bottom:3px">'
        +'<span style="font-size:12px;font-weight:600;color:var(--text)">'+p.name+'</span>'
        +'<span class="hand-tag '+handCls+'">'+hand+'HP</span>'
        +'</div>'
        +'<div style="display:flex;gap:8px">'
        +'<div style="font-size:10px;color:var(--muted2)">vs RHH <span class="hr9-val '+rCls+'" style="font-size:10px">'+(rhr9>0?rhr9.toFixed(1):'--')+'</span></div>'
        +'<div style="font-size:10px;color:var(--muted2)">vs LHH <span class="hr9-val '+lCls+'" style="font-size:10px">'+(lhr9>0?lhr9.toFixed(1):'--')+'</span></div>'
        +'</div>'
        +'</div>';
    }

    // ── Compact batter list ──
    function batterList(batters,pitcher,side,rowPrefix){
      if(!batters.length)return'<div style="font-size:11px;color:var(--muted2);padding:8px">No data</div>';
      // Check if lineup is confirmed
      var isConfirmed=batters.some(function(b){return b.lineup_source==='confirmed'||b.batting_slot>0;});
      var orderLabel=isConfirmed
        ?'<span style="color:var(--g);font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em">Confirmed Order</span>'
        :'<span style="font-size:9px;color:var(--muted2);text-transform:uppercase;letter-spacing:.06em">Projected Order</span>';

      var sec='<div style="padding:6px 8px">';
      // Column header
      sec+=orderLabel;
      sec+='<div style="display:grid;grid-template-columns:14px 1fr 38px 30px 30px 38px;gap:2px;padding:3px 0 4px;border-bottom:0.5px solid var(--border2);margin-top:4px">';
      sec+='<div style="font-size:8px;color:var(--muted2)"></div>';
      sec+='<div style="font-size:8px;color:var(--muted2)">Batter</div>';
      sec+='<div style="font-size:8px;color:var(--muted2);text-align:right">xSLG</div>';
      sec+='<div style="font-size:8px;color:var(--muted2);text-align:right">Brl</div>';
      sec+='<div style="font-size:8px;color:var(--muted2);text-align:right">LA</div>';
      sec+='<div style="font-size:8px;color:var(--muted2);text-align:right">HR%</div>';
      sec+='</div>';

      var showN=Math.min(batters.length,5);
      batters.slice(0,showN).forEach(function(b,bi){
        b._full=b; b._pitP=pitcher;
        var s=b.season||{};var l8=b.l8d||{};var bd=b.breakdown||{};
        var xslg=l8.xslg||bd.xslg_l8d||0;
        var brl=s.barrel||s.barrel_pct||bd.barrel_season||0;
        var la=s.la||bd.la_use||0;
        var pct=b.hr_prob||0;
        var pc=pctCls2(pct);
        var slot=b.batting_slot&&b.batting_slot>0?b.batting_slot:bi+1;
        var rowId=rowPrefix+'-'+bi;
        var isTop=bi===0;
        var rowBg=isTop?'background:rgba(50,215,75,0.06);border-radius:4px;':'';

        sec+='<div style="'+rowBg+'" onclick="toggleGcDetail(\''+rowId+'\')" style="cursor:pointer">';
        sec+='<div style="display:grid;grid-template-columns:14px 1fr 38px 30px 30px 38px;gap:2px;padding:3px 2px;cursor:pointer;align-items:center">';
        sec+='<div style="font-size:9px;color:var(--muted2);text-align:center">'+slot+'</div>';
        sec+='<div style="font-size:12px;font-weight:'+(isTop?'600':'400')+';color:'+(isTop?'var(--text)':'var(--muted)')+';white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+b.name+'</div>';
        sec+='<div style="font-size:10px;color:var(--muted);text-align:right">'+fmtXslg(xslg)+'</div>';
        sec+='<div style="font-size:10px;color:var(--muted);text-align:right">'+(brl>0?brl.toFixed(1)+'%':'--')+'</div>';
        sec+='<div style="font-size:10px;color:var(--muted);text-align:right">'+(la>0?la.toFixed(0)+'°':'--')+'</div>';
        sec+='<div class="bat-pct '+pc+'" style="font-size:12px;text-align:right">'+pct+'%</div>';
        sec+='</div>';
        sec+='<div class="gc-detail" id="'+rowId+'" style="display:none">'+buildBatDetail(b)+'</div>';
        sec+='</div>';
      });

      if(batters.length>showN){
        sec+='<div style="text-align:center;padding:4px;font-size:10px;color:var(--muted2);cursor:pointer;border:0.5px solid var(--border2);border-radius:4px;margin-top:4px" onclick="this.parentNode.querySelectorAll(\'.extra-bat\').forEach(function(e){e.style.display=e.style.display===\'none\'?\'block\':\'none\'});this.textContent=this.textContent.includes(\'+\')?\'show less\':\'+ '+(batters.length-showN)+' more\'">'+(batters.length-showN)+' more</div>';
        batters.slice(showN).forEach(function(b,bi){
          b._full=b; b._pitP=pitcher;
          var s=b.season||{};var l8=b.l8d||{};var bd=b.breakdown||{};
          var xslg=l8.xslg||bd.xslg_l8d||0;
          var brl=s.barrel||s.barrel_pct||bd.barrel_season||0;
          ...
          var pct=b.hr_prob||0;
          var pc=pctCls2(pct);
          var slot=b.batting_slot&&b.batting_slot>0?b.batting_slot:showN+bi+1;
          var rowId=rowPrefix+'-extra-'+bi;
          sec+='<div class="extra-bat" style="display:none">';
          sec+='<div style="display:grid;grid-template-columns:14px 1fr 38px 30px 30px 38px;gap:2px;padding:3px 2px;cursor:pointer;align-items:center" onclick="toggleGcDetail(\''+rowId+'\')">';
          sec+='<div style="font-size:9px;color:var(--muted2);text-align:center">'+slot+'</div>';
          sec+='<div style="font-size:12px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+b.name+'</div>';
          sec+='<div style="font-size:10px;color:var(--muted);text-align:right">'+fmtXslg(xslg)+'</div>';
          sec+='<div style="font-size:10px;color:var(--muted);text-align:right">'+(brl>0?brl.toFixed(1)+'%':'--')+'</div>';
          sec+='<div style="font-size:10px;color:var(--muted);text-align:right">'+(la>0?la.toFixed(0)+'°':'--')+'</div>';
          sec+='<div class="bat-pct '+pc+'" style="font-size:12px;text-align:right">'+pct+'%</div>';
    ...
    var rowBorder=gi>0?'border-top:0.5px solid var(--border)':'';
    h+='<div style="display:grid;grid-template-columns:210px 75px 85px 1fr 1fr;gap:0;border:0.5px solid var(--border);border-top:none">';

    // Col 1 - Game info
    h+='<div style="padding:10px 10px;'+rowBorder+'">';
    h+='<div style="font-size:11px;color:var(--muted2);margin-bottom:3px">'+timeStr+' ET</div>';
    h+='<div style="font-size:14px;font-weight:600;color:var(--text);margin-bottom:8px">'+awayTeam+' <span style="color:var(--muted2);font-weight:400;font-size:11px">@</span> '+homeTeam+'</div>';
    h+=pitcherInfo(awayP);
    h+='<div style="border-top:0.5px solid var(--border2);margin:6px 0"></div>';
    h+=pitcherInfo(homeP);
    h+='</div>';

    // Col 2 - Projected HRs
    h+='<div style="padding:10px 8px;border-left:0.5px solid var(--border);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px">';
    h+='<div style="font-size:24px;font-weight:600;color:'+projC+'">'+gameProj.toFixed(1)+'</div>';
    h+='<div style="font-size:10px;color:var(--muted2)">';
    h+='<span>'+awayProj.toFixed(1)+'</span><span style="color:var(--border)"> / </span><span>'+homeProj.toFixed(1)+'</span>';
    h+='</div>';
    h+='<div style="font-size:9px;font-weight:700;color:'+projC+'">'+projLabel+'</div>';
    h+='</div>';

    // Col 3 - Wind
    h+='<div style="padding:10px 8px;border-left:0.5px solid var(--border);display:flex;align-items:center;justify-content:center">';
    h+=windArrow(wx,isDome);
    h+='</div>';

    // Col 4 - Away batters
    h+='<div style="border-left:0.5px solid var(--border)">';
    h+=batterList(awayBatters,homeP,'away',gi+'-away');
    h+='</div>';

    // Col 5 - Home batters
    h+='<div style="border-left:0.5px solid var(--border)">';
    h+=batterList(homeBatters,awayP,'home',gi+'-home');
    h+='</div>';
  ...
  el.innerHTML='<div class="gc-wrap" style="border-radius:8px;overflow:hidden">'+h+'</div>';
}
